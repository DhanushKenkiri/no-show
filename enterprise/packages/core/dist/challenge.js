...
export function challengeAtBlock(eventId, blockNumber) {
    return keccak256(encodePacked(["bytes32", "uint256"], [eventId, blockNumber / CHALLENGE_BLOCKS]));
}
/** Compute the challenge for a window index directly. */
export function challengeForWindow(eventId, window) {
    return keccak256(encodePacked(["bytes32", "uint256"], [eventId, window]));
}
/** The window index a block falls in. */
export function windowOf(blockNumber) {
    return blockNumber / CHALLENGE_BLOCKS;
}
/** Blocks remaining in the window containing `blockNumber` — 3, 2 or 1. */
export function blocksLeftInWindow(blockNumber) {
 ...
 */
export function venueChallenge(eventId, blockNumber, lead = DEFAULT_VENUE_LEAD_WINDOWS) {
 ...
 */
export function deriveEventId(tenantId, externalEventId) {
    return keccak256(encodePacked(["string", "string", "string"], [tenantId, ":", externalEventId]));
}
